...
params.diameter = 10;
params.switch_step = zeros(params.n, params.steps); %1;
...
%% Extended BBS
switch_counter = 0;
params.actvie_agents = ones(1, params.n);

% Initializing prev_seq to avoid an error for the first usage
prev_seq = zeros(params.n, 2, params.h_bc+1);
...
for t = 1:params.steps    
    ...
    if mod(t - 1, controller_run) == 0
        ...
        if decision
            ...
            switch_counter = switch_counter + params.n;
            params.switch_step(:, t) = 1;
...
%% Print results
display(['Total switches: ' num2str(switch_counter)]);
[res_distance, avg_dist] = get_distance(pos, params.tgt);
display(res_distance);
display(avg_dist);
